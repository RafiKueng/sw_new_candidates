...
path     = join(S['output_dir'], 'mlens_vs_mstel_per_swid')
filename = 'mstel_vs_mtot_{swid}.png'
overwrite = True
...
SUBSET_MODELS, MAPS = CRDA.get_dataset_data('all_models')
...
#    if LENSES[asw]['z_lens'] <= 0:
#        print "skipping, no redshift available"

    for mid in MAPS['swid2mids'].get(swid, ""):  # hack we only want the first entry of the list
    ...
    # filename
    N = dict(mid=mid,swid=swid,asw=asw)
    ffn = join(path, filename.format(**N))
