...
#        imgname = join(fpath, filename.format(_={'asw':asw, 'mid':mid,'swid':swid}))
        ...
        print "      %.2f, %.2f, %.2f" % (r_rcf, m_rcf, k_rcf)
        ...
        for mM in range(len(ensem)):
            ensem1d = np.reshape(ensem[mM],(N**2))   #reshape 2D array as 1D array 
        ...
        for mM in range(len(ensem)):
            ...
            if mM==0:                                #create MoI tensor (outer products of pairs of values)
                outsum = out
            else:
                outsum = outsum + out
