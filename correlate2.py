...
with open("selected_models.txt", "w") as f:
    for mid, adata in selected_models.items():
        ...
        if len("%s"%mid)<6:
            ...
            iurl = "http://mite.physik.uzh.ch/result/%06i/input.png" % int(mid)
        else:
            urll = "http://labs.spacewarps.org/spaghetti/model/" + mid
            iurl = "http://labs.spacewarps.org/media/spaghetti/%s/%s/input.png" % (mid[0:2], mid[2:])
        f.write("         %s %s %10s %s\n" % (swid, asw, mid, urll))
...
totl = len(mids)
for jj, mid in enumerate(mids):
    
    print "plotting",jj, mid, 'of', totl, urlss[jj]
...
#        ax.scatter(_x, _y, c='w')
#        
#    
#    ax.scatter([x[jj]], [y[jj]], c=colormap2[categories[jj]])
#    
#    #axis limits
#    amin = 2e8
#    amax = 2e13
#    
#    plt.title("Stellar vs Lensing Mass (%s)" % str(mid) )
...
#    plt.close()
    ...
    with open(dirr+'/list.html', 'a') as f:
        f.write('\n<a href="%s">%s</a></br>\n' % (urlss[jj][0], urlss[jj][1]))
        f.write('<img src=%s alt="LensURL" /></br></br>\n\n' % iurls[jj])
